...
function getViewSourceHref(item, gitContribute, gitUrlPattern) {
    ...
    return getRemoteUrl(item.source.remote, item.source.startLine - '0' + 1, null, gitUrlPattern, null);
}
...
function getImproveTheDocHref(item, gitContribute, gitUrlPattern) {
    ...
    if (!item.documentation || !item.documentation.remote) {
        ...
        return getRemoteUrl(item.documentation.remote, item.documentation.startLine + 1, gitContribute, gitUrlPattern, item);
    }
}
...
var gitUrlPatternItems = {
    ...
    'adowiki': {
        ...
        'generateUrl': function (gitInfo, item) {
            ...
            if (item.wikiPath)
            {
                console.log("Using wikiPath: [" + item.wikiPath + "]");
                path = item.wikiPath;
            }
            else
            {
                ...
                console.log("url:[" + url + "]");
                console.log("relativepath: [" + gitInfo.relativePath + "]");
                console.log("path to work with: [" + gitInfo.path + "]");
                var relativePath = "/";
                ...
                {
                    console.warn("Relative Path not provided, using the default '/'. In docfx.json, under build/fileMetadata/_gitContribute/[]/ add a key value pair \"relativePath\": \"foo\"");
                }
...
function getRemoteUrl(remote, startLine, gitContribute, gitUrlPattern, item) {
...
function getGitInfo(gitContribute, gitRemote) {
    ...
    mergeKey(gitContribute, gitRemote, result, 'relativePath');
